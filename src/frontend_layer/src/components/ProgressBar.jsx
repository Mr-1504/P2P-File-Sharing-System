...
const ProgressBar = ({ tasks, setTasks, onResume }) => {
    ...
    const getTaskType = (task) => {
        if (task.status === 'sharing' || task.taskType === 'sharing' || task.taskType === 'share') {
            return 'sharing';
        }
        if (task.status === 'downloading' || task.taskType === 'downloading' || task.taskType === 'download') {
            return 'downloading';
        }
    ...
    const getStatusText = (status, taskType) => {
        console.log('Getting status text for', status, taskType);
        if (taskType === 'sharing') {
            switch (status) {
                ...
                <div className="space-y-4 max-h-[calc(100vh-300px)] overflow-y-auto">
                    {(() => {
                        const activeTasks = tasks.filter(task => {
                            ...
                            return type === 'sharing' || type === 'downloading';
                        });
                        activeTasks.sort((a, b) => parseInt(b.id) - parseInt(a.id));
                        return activeTasks.map(task => {
                            ...
                            console.log('Rendering task:', task, 'as type:', taskType, 'with status text:', statusText);
                                    ...
                                    {taskType === 'sharing' ? (
                                        ...
                                        <div className="bg-blue-50 rounded-lg p-2 text-center border border-blue-200">
                                            <div className="flex items-center justify-center space-x-2">
                                                ...
                                                <span className="text-xl font-bold text-blue-700">{task.progress}%</span>
                                                <span className="text-blue-500 text-xs">{t('completed')}</span>
                                            </div>
                                        </div>
                                    ) : (
